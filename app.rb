  ...
  route do |r|
    r.on('api') do
      r.on('v1') do
        ...
        r.post('check_auth') do
          # rubocop:disable Lint/Debugger
          #binding.pry
          HTTP.post('http://localhost:3001/store_data_user', json: r.params)
        end

        r.post('send_to_users') do
          # rubocop:disable Lint/Debugger
          #binding.pry
          #HTTP.post('http://localhost:3001/store_data_user', json: r.params)
        end

        r.post('start_check_id') do
          # rubocop:disable Lint/Debugger
          binding.pry
          #HTTP.post('http://localhost:8080/check_id', key: 72)
        end
